...
/**
 * Thin HTTP/WebSocket client that applies the quirks of the YoloBox Web Control API
 * (session cookies, auth heartbeat, retry logic) so the rest of the module can stay small.
 */
class YoloboxClient {
	constructor(logFn, host, port = 8887, options = {}) {
		...
		// Store cookies coming from the ws handshake so follow-up calls reuse the session.
	...
	async request(path, options = {}) {
		...
		if (!skipAuth) {
			// Make sure the WS session is authenticated before we attempt the request.
		...
		while (true) {
			try {
				...
				// The YoloBox closes the socket for a lot of transport errors; only retry the safe ones.
				const retriable =
				...
				if (retriable && attempt < maxRetries) {
					if (!skipAuth) {
						// Re-auth once more in case the cookie expired between attempts.
						await this.ensureAuthenticated(true)
					}
	...
	buildCookieHeader() {
		// Convert the cookie jar into the single header the WS client expects.
	...
	async sendOrder(orderId, data) {
		...
		// Every command is expressed as an order with an optional payload blob.
		const payload = {
			orderID: orderId,
			data: data ?? {},
		}
	...
	async fetchSceneImage(params = {}) {
		...
		return new Promise((resolve, reject) => {
			// Use the raw HTTP endpoint because the thumbnails are served as binary blobs.
